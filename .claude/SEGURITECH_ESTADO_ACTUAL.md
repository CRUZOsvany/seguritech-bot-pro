...
5. `scripts/smoke-test.sh` está desactualizado: referencia `npm run dev:backend` (no existe, el script real es `dev`) y pega contra `/webhook/test-tenant`, un tenant que no está sembrado en ningún seed. No usar hasta corregirlo (ver `docs/deployment/RUNBOOK_PRODUCCION.md` §7).
