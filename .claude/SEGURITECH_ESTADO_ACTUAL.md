...
### Panel (hallazgos del 2026-09-13)

| Qué falta | Detalle |
|---|---|
| Reactivar un cliente archivado | Sin endpoint ni pantalla. Hoy solo por SQL (`deleted_at = null`); el diálogo de archivar pide avisar |
| Vista de clientes archivados | No existe. Un archivado desaparece de todas las lecturas; solo el borrado permanente lo encuentra |
| Links del Designer a 2.16:1 | `tenants.$id.designer.tsx` usa `text-primary` (el dorado) como texto sobre crema en ~5 links de 10 px. El arreglo es `text-primary-text` |
| Designer enlazado desde WhatsApp | `tenants.$id.whatsapp.tsx` enlaza directo a `/tenants/$id/designer` (~l. 194 y 521), fuera del camino del Studio |
| Badges `fsm-*` sin modo oscuro | `badge.tsx`: sobre navy se ven como pastillas pastel claras. Preexistente |
| Tokens legacy en `globals.css` | `--color-bg`, `--color-surface`, `--color-primary-hover`… ya no los consume ningún componente, y su comentario dice lo contrario |
| CLI de shadcn | `npx shadcn add` no resuelve `@` (no hay `paths` en `frontend/tsconfig.json`). Se usa `--view` y se escribe a mano |

### Higiene y proceso

| ID | Qué falta |
|---|---|
| E-01 | **Tests de frontend: ya hay runner (vitest), con 13 archivos y 113 tests al 2026-09-13**, casi todos de modelos puros. No hay Testing Library instalado, así que componentes y hooks no se prueban. `[sin verificar]` cuánto del alcance de DEC-11 (ValidationPanel + serialización + hooks de TanStack Query) cubren |
...
| Fecha | Cambio |
|---|---|
| 2026-09-13 | **Cuarto de Control: Fases 1 y 2 del plan de mejora del panel** (#106, #107, #108; CI verde en `main` sobre `3822b66`). Archivar un cliente desde la tabla: el soft-delete existía y nunca se había conectado. Eliminarlo para siempre: `DELETE /api/admin/tenants/:id/permanent` hace hard-delete con cascade detrás de tres guardas —nombre escrito (con `trim`, sensible a mayúsculas, acentos y ñ), status `draft`/`sandbox`/`archived` y ningún `admin_operator` asignado—; también purga lo ya archivado (`findIncludingDeleted`) y audita `tenant.delete.permanent` aparte de `tenant.delete`. Además: toasts (sonner) en las mutaciones, buscador de clientes y paleta de marca navy/dorado/crema con contrastes calculados (dorado de texto `#7A6644`). La ficha del cliente tiene un solo botón de Studio y una barra de cinco secciones; el Designer queda solo como modo avanzado del Studio, también desde "Simular". Verificado: **101 suites · 995/995** en backend, **13 archivos · 113/113** en frontend, y E2E contra Cloud con tenants desechables (guardas de nombre y status, `trim`, archivar, purgar lo archivado, cascade y audit log). `[sin verificar en vivo]` la guarda del `admin_operator`: crear uno desechable exige SQL directo. Hallazgos: el CLI de shadcn no resuelve `@` en `frontend/` (su `tsconfig.json` no declara `paths`) y `Cerrajeria Tony` estaba guardado con un espacio al final. |
