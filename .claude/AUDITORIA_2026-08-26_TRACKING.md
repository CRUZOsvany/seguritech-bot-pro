...
**Estado:** ✅ **RESUELTO (2026-09-01).** Cris aplicó la migración. Verificado con la misma query de solo-lectura: `bot_users?select=last_inbound_at,opted_out_at` → `200`, ambas columnas existen.
...
| C-04 | `wait_input` sin validar | ✅ resuelto parcial: validación `numeric` en `wait_input` (`content.validation` + `content.validation_error`, con reintento amable que re-renderiza el nodo sin avanzar ni guardar contexto), aplicada a `pedido_cantidad` de `papeleria-flow.json`. Alcance chico a propósito — **no** es el sistema genérico de validación de flows, eso sigue siendo DEC-11/E-01. 6 tests en `FlowInterpreter.waitInputValidation.test.ts` |
...
| C-08 | Escape words hardcodeadas | ⬜ **sigue abierto.** La depuración motor+simulador (Fase 1) arregló la PRECEDENCIA del escape global (ahora solo aplica si el nodo actual no tiene transición propia para ese mensaje) — eso era un bug distinto. `ESCAPE_WORDS` sigue siendo una constante global en `FlowInterpreter.ts`, no configurable por tenant ni por flow: C-08 como tal no está resuelto |
...
| DEC-12 | ¿Rubros médico/farmacia en V1? | ✅ **Ofrecerlos con guardrail — ejecutado**, con el alcance real replanteado (ver nota abajo): nuevo `restrictedGiroCatalogGuardrail.ts`, invocado en `SupabaseBotFlowRepository.publishDraft` (no en `flowSchema.ts`, que valida JSON puro sin contexto de tenant/DB). Bloquea `POST .../flows/:flowId/publish` (400, `RestrictedGiroGuardrailError`) cuando el tenant es medico/farmacia, el flow expone `catalog_items` vía `send_list`, y el catálogo real del tenant tiene una categoría del blocklist (medicamento controlado/con receta/psicotrópico/etc — punto de partida razonable, no lista validada por experto legal, anotado explícitamente en el código). 14 tests nuevos (11 del validador puro + 3 de wiring en el repositorio). No implementa detección de "lenguaje de diagnóstico" en texto libre — decisión explícita, un filtro de palabras sobre prosa da falsos positivos/negativos constantes; queda como revisión humana obligatoria del molde, documentada en el código. |
...
| 2026-08-26 (cont. 4) | **DEC-12 implementada**, con el alcance real re-derivado en vez de seguir la redacción literal de la auditoría: "rechazar categorías en flowSchema.ts" no aplica porque flowSchema.ts valida JSON puro sin tenantId/DB, y los flows tampoco referencian categorías directamente (`items_source: 'catalog_items'` trae TODO el catálogo disponible, sin filtro por categoría). Guardrail real: nuevo `domain/validators/restrictedGiroCatalogGuardrail.ts`, invocado desde `SupabaseBotFlowRepository.publishDraft` (que sí tiene tenantId y acceso a DB) — consulta `tenants.giro` y `catalog_items.category` SOLO cuando el flow expone `catalog_items` (lazy, evita 2 queries de más en el camino común). Bloquea el publish con 400 + `RestrictedGiroGuardrailError` si hay categoría restringida. Detección de "lenguaje de diagnóstico" en texto libre explícitamente NO implementada (fragilidad de falsos positivos/negativos en un riesgo regulatorio real) — queda como revisión humana documentada, no automatizada. 14 tests nuevos (11 validador puro sin mocks + 3 wiring del repositorio, camino de rechazo). Suite: 47 suites, 344/344, typecheck y eslint limpios. Quedan sin implementar: DEC-02 (carrito), DEC-08 (delay), DEC-14 (Sentry/UptimeRobot), E-01 (tests frontend). |
| 2026-09-01 | **A-03-bis cerrado:** Cris aplicó la migración 019 en el SQL Editor de Supabase Dashboard. Verificado por lectura read-only contra `bot_users?select=last_inbound_at,opted_out_at` → `200`, columnas existen (`null` en filas existentes, esperado). Ya no bloquea el primer mensaje real de Meta. |
| 2026-09-03 | **Housekeeping de terminal** (a partir de un log de `npm run build:panel` + `npm run dev` + `npm ci` que Cris pegó para análisis): `npm audit fix` desde la raíz resolvió las 4 vulnerabilidades (`js-yaml` CVE-2026-59870 directo, `pm2` transitiva del mismo CVE, `browserslist` 2 GHSA, `postcss-selector-parser` DoS de AST) sin bump mayor — 0 vulnerabilidades tras el fix, confirmado con suite completa (47/47, 344/344) + build de `frontend` + typecheck, sin regresiones. Revisados y aprobados los 2 postinstall pendientes del gate `allowScripts` de npm: `msw@2.14.6` (transitiva de `shadcn` CLI, dev-only, no-op sin `msw.workerDirectory` en `package.json`) y `unrs-resolver@1.11.1` (transitiva de `jest-resolve`, descarga de binario nativo N-API, patrón estándar) — ninguno sospechoso. **Hallazgo nuevo documentado (no arreglado, no bloqueante):** dos catálogos paralelos en el código — `catalog_items` (legacy, alimenta `TenantConfig.catalog`) vs `pos_products` (real, 110+ SKUs, consultado en vivo por `CatalogSearchService`/nodo `search_catalog`). `catalogSize: 0` en el log de "Papelería DEMO" viene de `catalog_items` vacía, no de `pos_products`. `VariableResolver.selected_product_name/price` ya tiene fallback a `pos_products` y no rompe; `{{catalog_listing}}` y `DynamicSectionResolver` con `items_source: 'catalog_items'` NO tienen fallback y mostrarían "Aún no hay productos" con inventario real disponible — `papeleria-flow.json` no usa ninguno de los dos hoy, así que no es un bug activo. Advertencia agregada como comentario en `SupabaseTenantConfigService.ts`. Nada de esto es un DEC/hallazgo de la auditoría original — es deuda nueva, sin ID asignado todavía. |
| 2026-09-05 | **Depuración motor + fidelidad del simulador, Fases 1-4** (rama `fix/motor-simulador-fases-1-4`, PR pendiente de abrir). **Fase 1:** invertida la precedencia de `ESCAPE_WORDS` en `FlowInterpreter.execute` — el escape global ahora solo aplica si el nodo actual NO tiene una transición propia (distinta de `default`) para ese mensaje. Arregla dos bugs reales de `papeleria-flow.json`: "cancelar" en `pedido_confirma` ya no borra el pedido entero (cae en la transición local que preserva `selected_product_id`), y `{keyword:['salir',…]} → despedida` en `bienvenida` dejó de ser código muerto. El pre-chequeo excluye explícitamente `catalog_not_found`: esa condición es TRUE por ausencia de cómputo (el pre-chequeo no corre `CatalogSearchService`), no porque una búsqueda real haya fallado, así que sin la exclusión cualquier palabra de escape en un nodo `search_catalog` (p.ej. "menu" estando en `buscar`) quedaba absorbida como "búsqueda sin resultado" en vez de resetear. **Fase 2:** el formato de la alerta al dueño se extrajo de `BotController.enrichOwnerAlert` (método privado) a `domain/services/OwnerAlertFormatter.ts`, y `SimulateMessageUseCase` ahora lo aplica — el panel muestra el texto REAL que recibiría el dueño (wa.me + hora + `#listo XXXX`), no el `owner_alert_template` crudo; `WhatsAppSimulator.tsx` lo renderiza en un bloque de "alerta interna" en vez del texto fijo de antes. **Fase 3:** el TTL de sesión (DEC-07) se extrajo a `domain/services/SessionTtlPolicy.ts`, compartido entre `BotController` (mensajes reales, comportamiento sin cambios — sus 7 tests siguen en verde sin tocarlos) y `SimulateMessageUseCase`, que lo dispara con el campo nuevo `simulatedElapsedMinutes` (botón "⏳ Saltar 2h de inactividad" en el panel). **Fase 4:** validación `numeric` en `wait_input` — cierra C-04 parcial, ver esa fila. C-08 explícitamente NO cerrado: `ESCAPE_WORDS` sigue siendo constante global no configurable por tenant. 4 suites nuevas, 20 tests nuevos. Suite: 51 suites, 364/364, typecheck backend+frontend limpio, eslint sin errores. **Fuera del guion del prompt, necesario para que la Fase 3 funcione de punta a punta:** `tenantsRouter.ts` (POST /api/admin/simulate) destructura el body campo por campo, así que hubo que agregar ahí el pass-through + validación de `simulatedElapsedMinutes`; sin eso el botón del panel mandaba un campo que la API descartaba. |
